...
export class DatabaseStorage implements IStorage {
  ...
  async getIamAccounts(): Promise<IamAccount[]> {
    return await db.select().from(iamAccounts).orderBy(desc(iamAccounts.createdAt));
  }
  ...
  async createIamAccount(data: Partial<IamAccount>): Promise<IamAccount> {
    const accountData: Partial<IamAccount> = {
    ...
    const [newAccount] = await db.insert(iamAccounts).values(accountData).returning();
    return newAccount;
  }
