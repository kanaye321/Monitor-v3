...
export default function IAMAccounts() {
  ...
  const { data: iamAccounts = [], isLoading } = useQuery({
    queryKey: ['/api/iam-accounts'],
    queryFn: async () => {
      const response = await apiRequest('/api/iam-accounts');
      return response;
    }
  });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between gap-4">
        <div className="relative max-w-md w-full">
          ...
          <Input
            placeholder="Search by requestor, Knox ID, or approval ID..."
            className="pl-10"
            value={searchTerm}
